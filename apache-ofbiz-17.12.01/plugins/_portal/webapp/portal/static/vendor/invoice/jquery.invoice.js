...
Invoice.prototype = {
     ...
     */
    newRow: function () {
            jQuery(".item-row:last").each(function (i) {
                var row = jQuery(this);
                var price = row.find($.opt.price).val();
                if (row.find(".item").val().length != 0 && (price.length != 0 && row.find($.opt.qty).val().length != 0)){
                    jQuery(".item-row:last").after('<tr class="item-row"><td class="item-name"><div class="delete-btn"><input type="text" class="form-control form-control-sm item" placeholder="Item" type="text"></div></td><td><input class="form-control form-control-sm price" placeholder="Price" type="text"> </td><td><input class="form-control form-control-sm qty" placeholder="Quantity" type="text"></td><td><span class="total">0.00</span></td><td><a class=""  href="javascript:;" title="Remove row"><i class="fa fa-trash-o text-danger" id="delete"></i> </a></td></td></tr>');
                    row.find($.opt.price).removeClass('border-danger')
                    $(row.find($.opt.qty)).removeClass('border-danger')
                    row.find(".item").removeClass('border-danger')
                }else {

                    if (price.length == 0){
                        $(row.find($.opt.price)).addClass('border-danger')
                    }else {row.find($.opt.price).removeClass('border-danger')}
                    if (row.find($.opt.qty).val().length == 0){
                        $(row.find($.opt.qty)).addClass('border-danger')
                    }else {$(row.find($.opt.qty)).removeClass('border-danger')}
                    if (row.find(".item").val().length == 0){
                        row.find(".item").addClass('border-danger')
                    }else {row.find(".item").removeClass('border-danger')}
                }
            })
